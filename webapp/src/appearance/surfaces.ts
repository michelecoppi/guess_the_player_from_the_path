...
export function renderAppearanceIdentity(name: string, value: unknown): string {
  ...
  return `${renderAvatar({ name, tactics: a.frame.tactics, ringMotion: a.frame.motion, ringStyle: a.frame.ring ? `background: ${a.frame.ring}` : undefined })}<div class="cosmetic-identity"><b>${e(name)}</b>${a.badge ? `<span class="cosmetic-badge">${e(a.badge)}</span>` : ""}${a.title.label ? `<p class="cosmetic-title"${a.title.color ? ` style="border-color: ${a.title.color}"` : ""}>${e(a.title.label)}</p>` : ""}${a.number ? `<span class="cosmetic-number">Nº ${e(a.number)}</span>` : ""}</div>`;
}
