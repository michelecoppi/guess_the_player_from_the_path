...
export function eventsFixture(): EventsState {
  return {status:'ready',selectedCode:null,feedback:null,draftAnswer:'',error:null,events:['path','career','father_son','transfer_guess','blind_path','link_club'].map((type,i)=>({
    code:`review-event-${i}`,day:'2026-09-13',type,name:['Il regista','Una carriera, tanti club','Di padre in figlio','L’ultimo trasferimento','Carriera al buio','Trova il collegamento'][i]!,description:'Una nuova sfida nella storia del calcio.',dates:['2026-09-13','2026-09-20'],available:true,rules:'Tre tentativi. Un punto bonus alla prima risposta corretta.',player_name:type==='career'?'Andrea Pirlo':'',player_names:type==='link_club'?['Andrea Pirlo','Gianluigi Buffon']:[],min_correct:3,career_path:type==='path'?career:type==='blind_path'?career.slice(-1):[],total_stops:type==='blind_path'?5:null,image_url:null,points:5,bonus_available:true,progress:{attempts:0,revealed:1,revision:0,finished:false,solved:false,points:0},leaderboard:[{name:'Giulia',points:16},{name:'Luca',points:12}] }))};
}
