...
| 🎊 Eventi (`events.py`) | Status, per-day content, missing days, participants ranking; event templates with validity, schedule and candidates | Activate/deactivate, move dates, answers, bonus, delete, create manual event; create/edit a template as JSON with validation, preview and backup ([event-templates.md](event-templates.md)) |
