...
| `feature_flags.config.loaded`, `feature_flags.refresh.failed`, `feature_flags.config.rejected`, `feature_flags.config.invalid`, `feature_flags.change.applied` | INFO / WARNING | Feature flag cache and operator tool; never target ids. See [feature-flags.md § Observability](feature-flags.md#observability). |
